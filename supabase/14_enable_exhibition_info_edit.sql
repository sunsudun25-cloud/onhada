...
BEGIN
    ...
    -- cover_image_path는 NULL(대표 이미지 없음)을 허용하되, 값이 있다면
    -- 반드시 "이 전시관 자신의 UUID 폴더 + 무작위 UUID 파일명" 형식이어야
    -- 한다. 슬래시가 2개(경로 구분자 1개)를 초과하는 값, 폴더가 다른
    -- 전시관 UUID인 값, 원본 파일명을 쓴 값, "..","//" 등 경로 탐색
    -- 시도는 이 정규식 하나로 전부 거부된다(POSIX 정규식은 앵커(^...$)로
    -- 전체 문자열을 강제하므로 부분 일치로 우회할 수 없다). 대소문자는
    -- 의도적으로 소문자만 허용한다(대소문자 구분 없는 !~* 대신 !~ 사용) -
    -- 실제 경로는 항상 Postgres UUID::text 캐스팅과 crypto.randomUUID()가
    -- 만드는 소문자로만 생성되므로, 대문자 변형을 허용해도 실제로 존재하는
    -- Storage 객체를 가리킬 수 없다(대소문자를 구분하는 storage_exhibition_
    -- cover_select_public 정책의 등호 비교 때문에 조용히 실패할 뿐 보안
    -- 우회는 아니다). 다만 그런 값을 DB에 저장 자체를 허용할 이유가 없어
    -- 처음부터 소문자만 통과시킨다.
